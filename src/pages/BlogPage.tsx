    ...
    useEffect(() => {
        const getPosts = async () => {
            try {
                ...
                // Try 'blog' category slug first, fallback to fetching all posts
                let blogCategoryId = await fetchCategoryBySlug('blog');
                if (!blogCategoryId) {
                    blogCategoryId = await fetchCategoryBySlug('blogs');
                }
                const data = blogCategoryId ? await fetchPostsByCategory(blogCategoryId) : [];
                setPosts(data);
            } catch (error) {
        ...
        <div className="news-page">
            <section className="page-hero bg-primary">
                <div className="container">
                    <h1>Blog</h1>
                    <p>Insights, innovations, and stories from the world of Bioryth.</p>
                </div>
            </section>
